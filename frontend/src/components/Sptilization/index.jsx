...
import { faSearch, faStar, faCheckCircle, faMoneyBill } from '@fortawesome/free-solid-svg-icons';


function Index() {
    const dispatch = useDispatch();
    const clinic = useSelector(state => state.clinicSpecialization.spatlization);
    ...
    return (
        <div>
            ...
            <div className="text-container" style={{ textAlign: 'center' }}>
                ...
                <Carousel activeIndex={currentIndex} onSelect={handleSelect} style={{ margin: '0 auto', width: '80%' }}>
                    {clinic.map((item, index) => (
                        index % itemsPerSlide === 0 ? (
                            <Carousel.Item key={index}>
                                <div className="d-flex justify-content-between">
                                    {clinic.slice(index, index + itemsPerSlide).map((specialization, subIndex) => (
                                        <div key={subIndex} className="text-center" style={{ width: '30%' }}>
                                            <div className="card" style={{ width: '90%', margin: 'auto' }}>
                                                ...
                                                <div className="card-body">
                                                    <h5 className="card-title">{specialization.name_specialization}</h5>
                                                </div>
                                            </div>
                                        </div>
                                    ))}
            ...
            <div className="additional-info-container">
                <div className="additional-info" style={{ textAlign: 'right', color: '#333' }}>
                    ...
                    <div className="info-item">
                        <FontAwesomeIcon icon={faStar} className="info-icon" />
                        <span>تقييمات الدكاترة من مرضى حجزوا على فيزيتا و زاروا الدكتور بالفعل.</span>
                    </div>
                    <div className="info-item">
                    ...
                    <div className="info-item">
                        <FontAwesomeIcon icon={faMoneyBill} className="info-icon" />
                        <span>سعر الكشف على فيزيتا نفس سعر الكشف في العيادة، بدون أي مصاريف إضافية.</span>
                    </div>
                </div>
            </div>
        </div>
