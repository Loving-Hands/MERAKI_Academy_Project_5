  ...
  pool
    ...
    .then((result) => {
      if (result.rows.length) {
        ...
        bcryptjs.compare(password, admin.password, (err, response) => {
...
exports.deleteClinicById = async (req, res) => {
  try {
    ...
    const appointmentsResult = await pool.query(
      `SELECT id FROM "public"."appointment" WHERE clinic_id = $1`,
      [clinicId]
    );
    ...
    if (appointmentsResult.rowCount > 0) {
      ...
      await pool.query(
        `DELETE FROM "public"."appointment" WHERE id IN (${appointmentIds.join(
          ","
        )})`
      );
    }
    ...
    const clinicDeleteResult = await pool.query(
      `DELETE FROM "public"."clinics" WHERE id = $1`,
      [clinicId]
    );
    ...
    if (error.code === "23503") {
      ...
      res.status(400).json({
        success: false,
        message:
          "Cannot delete clinic because it is associated with existing appointments",
      });
    } else {
      // Other error
  ...
  try {
    ...
    if (error.code === "23503") {
      ...
      res.status(400).json({
        success: false,
        message:
          "Cannot delete user because it is associated with existing appointments",
      });
    } else {
      // Other error
      console.error("Error deleting clinic:", error);
...
exports.deletespecializationById = async (req, res) => {
  try {
    ...
    const result = await pool.query(
      `DELETE FROM "public"."specialization" WHERE id = $1`,
      [specializationId]
    );
    ...
    if (error.code === "23503") {
      ...
      res.status(400).json({
        success: false,
        message:
          "Cannot delete Specialization because it is associated with existing appointments",
      });
    } else {
      // Other error
      console.error("Error deleting clinic:", error);
